...
public class TestMoneroWalletRpc
{
    ...
    // instance variables
    protected IMoneroWallet wallet; // wallet instance to test

    public TestMoneroWalletRpc()
    {
        daemon = TestUtils.GetDaemonRpc();
        wallet = new MoneroWalletRpc(TestUtils.WALLET_RPC_URI);
    }
    ...
    {
        MoneroWalletConfig config = new MoneroWalletConfig().SetPath(path).SetPassword(password);
        IMoneroWallet ret;
        // assign defaults
        if (config == null)
        {
    ...
    protected Task<IMoneroWallet> CreateWallet(MoneroWalletConfig config)
    {
    ...
    protected Task CloseWallet(IMoneroWallet walletInstance, bool save)
    {
        throw new NotImplementedException();
    }

    protected Task<List<string>> GetSeedLanguages()
    {
        throw new NotImplementedException();
    }
    ...
    [Fact(Skip = "The method or operation is not implemented.")]
    ...
    [Fact(Skip = "The given key '0' was not present in the dictionary.")]
    ...
    [Fact(Skip = "Monero.Common.MoneroError : No wallet file")]
    ...
    [Fact(Skip = "MoneroWalletRpc.GetSeedLanguage() not supported")]
