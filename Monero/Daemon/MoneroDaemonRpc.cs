...
public class MoneroDaemonRpc : MoneroDaemonDefault
{
    ...
    public override string GetBlockHash(ulong height)
    {
        ...
        var respMap = rpc!.SendJsonRequest("on_get_block_hash", param);
    ...
    public override List<MoneroOutput> GetOutputs(List<MoneroOutput> outputs)
    {
        ...
        var resp = rpc.SendJsonRequest("get_outs", parameters);
        var result = resp.Result!;
    ...
    private static void CheckResponseStatus(Dictionary<string, object>? resp)
    {
        ...
        if (!"".Equals(status) && !"OK".Equals(status)) throw new MoneroRpcError(status);
    }
    ...
    private static MoneroTx? ConvertRpcTx(Dictionary<string, object>? rpcTx, MoneroTx? tx = null)
    {
        ...
        foreach (string key in rpcTx.Keys)
        {
            ...
            else if (key.Equals("signatures")) tx.SetSignatures(GenUtils.Reconcile(tx.GetSignatures(), ((JArray)val).ToObject<List<string>>()));
    ...
    private static MoneroPeer ConvertRpcConnection(Dictionary<string, object>? rpcConnection)
    {
        ...
        foreach (string key in rpcConnection.Keys)
        {
            ...
            else if (key.Equals("address_type"))
            {
                ...
                if (rpcType == 0) peer.SetConnectionType(MoneroConnectionType.INVALID);
                else if (rpcType == 1) peer.SetConnectionType(MoneroConnectionType.IPV4);
                else if (rpcType == 2) peer.SetConnectionType(MoneroConnectionType.IPV6);
                else if (rpcType == 3) peer.SetConnectionType(MoneroConnectionType.TOR);
                else if (rpcType == 4) peer.SetConnectionType(MoneroConnectionType.I2P);
    ...
    private static MoneroAltChain ConvertRpcAltChain(Dictionary<string, object> rpcChain)
    {
        ...
        foreach (string key in rpcChain.Keys)
        {
            ...
            else if (key.Equals("wide_difficulty")) { chain.SetDifficulty(GenUtils.Reconcile(chain.GetDifficulty(), PrefixedHexToUulong((string)val))); }
